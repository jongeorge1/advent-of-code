...
public class Part02 : ISolution
{
    public string Solve(string[] input)
    {
        ...
        int bestSeenFinishScore = int.MaxValue;

        List<PathState> bestRoutes = new();
        ...
        while (queue.Count > 0)
        {
            ...
            if ((bestSeenScores.TryGetValue((current.Location, current.Direction), out int bestScore) && bestScore < current.Score) || bestSeenFinishScore < current.Score)
            {
                continue;
            }
            ...
            {
                bestRoutes.Add(current);
                bestSeenFinishScore = current.Score;
                continue;
            }
        ...
        (int X, int Y)[] uniqueVisitedLocations = bestRoutes.SelectMany(x => x.VisitedLocations).Distinct().ToArray();
        int visitedLocationCount = uniqueVisitedLocations.Length + 1;
        return visitedLocationCount.ToString();
    }
